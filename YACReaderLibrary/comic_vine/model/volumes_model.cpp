...
VolumesModel::~VolumesModel() {
  // std::for_each(_data.begin(), _data.end(), [](QList<QString> * ptr) { delete
  // ptr; });
}
...
void VolumesModel::load(const QString &json) {
  ...
  while (it.hasNext()) {
    ...
    QString publisher =
        resultsValue.value("publisher").toMap().value("name").toString();
    QString url =
        resultsValue.value("image").toMap().value("medium_url").toString();
    ...
    test = name.isEmpty() && year.isEmpty() && numIssues.isEmpty() &&
           url.isEmpty();
...
QVariant VolumesModel::headerData(int section, Qt::Orientation orientation,
                                  int role) const {
...
QModelIndex VolumesModel::index(int row, int column,
