...
SelectVolume::SelectVolume(QWidget *parent)
  ...
  QLabel *label =
      new QLabel(tr("Please, select the right series for your comic."));
  ...
  cover->setStyleSheet("QLabel {background-color: #2B2B2B; color:white; "
  ...
  connect(tableVolumes->horizontalHeader(), SIGNAL(sectionClicked(int)),
          tableVolumes, SLOT(sortByColumn(int)));
  // connections
  connect(tableVolumes, SIGNAL(clicked(QModelIndex)), this,
          SLOT(loadVolumeInfo(QModelIndex)));
...
void SelectVolume::loadVolumeInfo(const QModelIndex &omi) {
  ...
  connect(comicVineClient, SIGNAL(seriesCover(const QByteArray &)), this,
          SLOT(setCover(const QByteArray &)));
  ...
  connect(comicVineClient2, SIGNAL(seriesDetail(QString)), this,
          SLOT(setDescription(QString)));
  connect(comicVineClient2, SIGNAL(finished()), comicVineClient2,
          SLOT(deleteLater()));
...
void SelectVolume::setDescription(const QString &jsonDetail) {
  QJsonParseError Err;
  QVariantMap sc =
      QJsonDocument::fromJson(jsonDetail.toUtf8(), &Err).toVariant().toMap();
  ...
  detailLabel->setText(
      valid ? descriptionValues.toString().replace(
                  "<a", "<a style = 'color:#827A68; text-decoration:none;'")
            : tr("description unavailable"));
}

QString SelectVolume::getSelectedVolumeId() {
  return model->getVolumeId(
      proxyModel->mapToSource(tableVolumes->currentIndex()));
}

int SelectVolume::getSelectedVolumeNumIssues() {
  return model->getNumIssues(
      proxyModel->mapToSource(tableVolumes->currentIndex()));
}
