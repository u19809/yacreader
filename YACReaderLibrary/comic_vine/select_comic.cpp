...
SelectComic::SelectComic(QWidget *parent) : ScraperSelector(parent), model(0) {
  ...
  // connections
  connect(tableComics, SIGNAL(clicked(QModelIndex)), this,
          SLOT(loadComicInfo(QModelIndex)));
...
void SelectComic::loadComicInfo(const QModelIndex &mi) {
  ...
  connect(comicVineClient, SIGNAL(comicCover(const QByteArray &)), this,
  ...
  connect(comicVineClient2, SIGNAL(comicDetail(QString)), this,
