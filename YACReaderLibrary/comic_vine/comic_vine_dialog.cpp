...
ComicVineDialog::ComicVineDialog(QWidget *parent) : QDialog(parent) {
...
void ComicVineDialog::doLayout() {
  ...
  QString dialogButtonsStyleSheet =
      "QPushButton {border: 1px solid #242424; background: #2e2e2e; "
      "color:white; padding: 5px 26px 5px 26px; "
...
void ComicVineDialog::doConnections() {
  ...
  connect(selectVolumeWidget, SIGNAL(loadPage(QString, int)), this,
          SLOT(searchVolume(QString, int)));
  connect(selectComicWidget, SIGNAL(loadPage(QString, int)), this,
          SLOT(getVolumeComicsInfo(QString, int)));
  connect(sortVolumeComicsWidget, SIGNAL(loadPage(QString, int)), this,
          SLOT(getVolumeComicsInfo(QString, int)));
...
void ComicVineDialog::goNext() {
  ...
  if (content->currentWidget() == seriesQuestionWidget) {
    if (seriesQuestionWidget->getYes()) {
      ...
      titleHeader->setSubTitle(tr("comic %1 of %2 - %3")
                                   .arg(currentIndex + 1)
                                   .arg(comics.length())
                                   .arg(title));
    ...
    // ComicDB-ComicVineID
    QList<QPair<ComicDB, QString>> matchingInfo =
        sortVolumeComicsWidget->getMatchingInfo();
    ...
    int count = selectVolumeWidget->getSelectedVolumeNumIssues();
    QString publisher = selectVolumeWidget->getSelectedVolumePublisher();
    QtConcurrent::run(this, &ComicVineDialog::getComicInfo, comicId, count,
                      publisher);
  }
}
...
QSize ComicVineDialog::minimumSizeHint() const { return QSize(872, 529); }
...
void ComicVineDialog::doLoading() {
  ...
  loadingMessage->setStyleSheet(
      "QLabel {color:white; font-size:12px;font-family:Arial;}");
...
void ComicVineDialog::debugClientResults(const QString &string) {
  ...
  // QMessageBox::information(0,"Result", QString("Number of results :
  // %1").arg(p.getNumResults()));
  if (p.responseError()) {
    QMessageBox::critical(0, tr("Error connecting to ComicVine"),
                          p.errorDescription());
    goBack();
  } else {
    switch (mode) {
...
void ComicVineDialog::queryTimeOut() {
  QMessageBox::warning(this, "Comic Vine error",
                       "Time out connecting to Comic Vine");
...
void ComicVineDialog::getComicsInfo(
    QList<QPair<ComicDB, QString>> &matchingInfo, int count,
  ...
  foreach (p, matchingInfo) {
    ...
    bool error;
    bool timeout;
    QByteArray result = comicVineClient->getComicDetail(
        p.second, error, timeout); // TODO check timeOut or Connection error
    if (error || timeout)
      continue; // TODO
    ComicDB comic = parseComicInfo(p.first, result, count,
                                   publisher); // TODO check result error
    comic.info.comicVineID = p.second;
    comics.push_back(comic);

    setLoadingMessage(
        tr("Retrieving tags for : %1").arg(p.first.getFileName()));
  }
  QString connectionName = "";
  {
    ...
    db.open();
    db.transaction();
    foreach (ComicDB comic, comics) { DBHelper::update(&(comic.info), db); }
    db.commit();
...
void ComicVineDialog::getComicInfo(const QString &comicId, int count,
                                   const QString &publisher) {
  ...
  bool timeout;
  QByteArray result = comicVineClient->getComicDetail(
      comicId, error, timeout); // TODO check timeOut or Connection error
  if (error || timeout) {
    // TODO
  ...
  ComicDB comic = parseComicInfo(comics[currentIndex], result, count,
                                 publisher); // TODO check result error
  comic.info.comicVineID = comicId;
  setLoadingMessage(
      tr("Retrieving tags for : %1").arg(comics[currentIndex].getFileName()));
  QString connectionName = "";
  {
    QSqlDatabase db = DataBaseManagement::loadDatabase(databasePath);
    db.open();
...
ComicDB ComicVineDialog::parseComicInfo(ComicDB &comic, const QString &json,
                                        int count, const QString &publisher) {
  QJsonParseError Err;

  QVariantMap sc =
      QJsonDocument::fromJson(json.toUtf8(), &Err).toVariant().toMap();
  if (Err.error != QJsonParseError::NoError) {
    qDebug("Error detected");
    return comic;
  }

  int numResults = sc.value("number_of_total_results")
                       .toInt(); // fix to weird behaviour using hasNext

  if (numResults > 0) {
    QVariantMap result = sc.value("results").toMap();
    comic.info.title = result.value("name").toString();
    comic.info.number = result.value("issue_number").toString();
    comic.info.volume = result.value("volume").toMap().value("name").toString();

    if (result.contains("person_credits")) {
      QMap<QString, QString> authors =
          getAuthors(result.value("person_credits"));
    ...
    if (result.contains("story_arc_credits")) {
      QPair<QString, QString> storyArcIdAndName =
          getFirstStoryArcIdAndName(result.value("story_arc_credits"));
      ...
      if (!storyArcId.isNull()) {
        ...
        QPair<QString, QString> arcNumberAndArcCount =
            getArcNumberAndArcCount(storyArcId, comicId);
...
QString ComicVineDialog::getCharacters(const QVariant &json_characters) {
  ...
  QMapIterator<QString, QVariant> it(json_characters.toMap());
  QVariantMap resultsValue;
  while (it.hasNext()) {
    it.next();
    resultsValue = it.value().toMap();

    Cs << resultsValue.value("name").toString();
  }

  return (Cs.isEmpty()) ? "" : (Cs.join("\n") + "\n");
}

QMap<QString, QString>
ComicVineDialog::getAuthors(const QVariant &json_authors) {
  QMap<QString, QString> authors;

  QMapIterator<QString, QVariant> it(json_authors.toMap());
  QVariantMap resultsValue;
  while (it.hasNext()) {
    it.next();
    resultsValue = it.value().toMap();
...
ComicVineDialog::getFirstStoryArcIdAndName(const QVariant &json_story_arcs) {
  QString story_arc_id = QString();
  QString story_arc_name = QString();

  QMapIterator<QString, QVariant> it(json_story_arcs.toMap());
  QVariantMap resultsValue;
  while (it.hasNext()) {
    it.next();
    resultsValue = it.value().toMap();
...
QPair<QString, QString>
ComicVineDialog::getArcNumberAndArcCount(const QString &storyArcId,
                                         const QString &comicId) {
  auto comicVineClient = new ComicVineClient;
  bool error;
  bool timeout;
  QByteArray result =
      comicVineClient->getStoryArcDetail(storyArcId, error, timeout);
  if (error || timeout)
  ...
  if (numResults > 0) {
    ...
    if (result.contains("issues")) {
      QMapIterator<QString, QVariant> it(result.value("issues").toMap());
      int arcNumber = 0;
      int arcCount = 0;
...
void ComicVineDialog::searchVolume(const QString &v, int page) {
  ...
  auto comicVineClient = new ComicVineClient;
  connect(comicVineClient, SIGNAL(searchResult(QString)), this,
          SLOT(debugClientResults(QString)));
  connect(comicVineClient, SIGNAL(timeOut()), this, SLOT(queryTimeOut()));
  connect(comicVineClient, SIGNAL(finished()), comicVineClient,
          SLOT(deleteLater()));
...
void ComicVineDialog::getVolumeComicsInfo(const QString &vID, int /* page */) {
  ...
  auto comicVineClient = new ComicVineClient;
  if (mode == Volume)
    connect(comicVineClient, SIGNAL(volumeComicsInfo(QString)), this,
            SLOT(showSortVolumeComics(QString)));
  else
    connect(comicVineClient, SIGNAL(volumeComicsInfo(QString)), this,
            SLOT(showSelectComic(QString)));
  connect(comicVineClient, SIGNAL(timeOut()), this, SLOT(queryTimeOut()));
  connect(comicVineClient, SIGNAL(finished()), comicVineClient,
          SLOT(deleteLater()));
