...
Viewer::Viewer(QWidget *parent)
    : QScrollArea(parent), fullscreen(false), information(false),
      doublePage(false), doubleMangaPage(false), zoom(100),
      currentPage(nullptr), wheelStop(false), direction(1), drag(false),
      numScrollSteps(22), shouldOpenNext(false), shouldOpenPrevious(false),
      magnifyingGlassShowed(false), restoreMagnifyingGlass(false) {
  ...
  palette.setColor(backgroundRole(),
                   Configuration::getConfiguration().getBackgroundColor());
  setPalette(palette);
  //---------------------------------------
  mglass = new MagnifyingGlass(
      Configuration::getConfiguration().getMagnifyingGlassSize(), this);
  mglass->hide();
  ...
  QSettings *settings = new QSettings(
      YACReader::getSettingsPath() + "/YACReader.ini", QSettings::IniFormat);
  ...
  if ((settings->value(USE_OPEN_GL).toBool() == true))
    goToFlow =
        new GoToFlowGL(this, Configuration::getConfiguration().getFlowType());
  else
    goToFlow =
        new GoToFlow(this, Configuration::getConfiguration().getFlowType());
#else
  goToFlow =
      new GoToFlow(this, Configuration::getConfiguration().getFlowType());
#endif
  ...
  // animations
  verticalScroller =
      new QPropertyAnimation(verticalScrollBar(), "sliderPosition");
  connect(verticalScroller, SIGNAL(valueChanged(const QVariant &)), this,
          SIGNAL(backgroundChanges()));
  horizontalScroller =
      new QPropertyAnimation(horizontalScrollBar(), "sliderPosition");
  connect(horizontalScroller, SIGNAL(valueChanged(const QVariant &)), this,
          SIGNAL(backgroundChanges()));
...
void Viewer::createConnections() {
  ...
  // goToDialog
  connect(goToDialog, SIGNAL(goToPage(unsigned int)), this,
          SLOT(goTo(unsigned int)));

  // goToFlow goTo
  connect(goToFlow, SIGNAL(goToPage(unsigned int)), this,
          SLOT(goTo(unsigned int)));
  ...
  connect(render, SIGNAL(errorOpening()), this,
          SLOT(showMessageErrorOpening()));
  connect(render, SIGNAL(errorOpening(QString)), this,
          SLOT(showMessageErrorOpening(QString)));
  connect(render, SIGNAL(crcError(QString)), this,
          SLOT(processCRCError(QString)));
  connect(render, SIGNAL(numPages(unsigned int)), goToFlow,
          SLOT(setNumSlides(unsigned int)));
  connect(render, SIGNAL(numPages(unsigned int)), goToDialog,
          SLOT(setNumPages(unsigned int)));
  // connect(render,SIGNAL(numPages(unsigned
  // int)),this,SLOT(updateInformation()));
  connect(render, SIGNAL(imageLoaded(int, QByteArray)), goToFlow,
          SLOT(setImageReady(int, QByteArray)));
  ...
  connect(render, SIGNAL(currentPageIsBookmark(bool)), this,
...
void Viewer::showGoToDialog() { goToDialog->open(); }
...
void Viewer::updateContentSize() {
  ...
  if (currentPage != nullptr && !currentPage->isNull()) {
    ...
    if (zoom != 100) {
      pagefit.scale(floor(pagefit.width() * zoom / 100.0f), 0,
    ...
    {
      QPixmap page =
          currentPage->scaled(content->width() * devicePixelRatio(),
                              content->height() * devicePixelRatio(),
...
void Viewer::scrollForwardHorizontalFirst() {
  if (!doubleMangaPage) {
    scrollZigzag(RIGHT, DOWN,
                 true); // right->right->lower left->right->...->next page
  } else {
    scrollZigzag(LEFT, DOWN,
                 true); // left->left->lower right->left->...->next page
  }
}

void Viewer::scrollBackwardHorizontalFirst() {
  if (!doubleMangaPage) {
    scrollZigzag(LEFT, UP,
                 false); // left->left->upper right->left->...->prev page
  } else {
    scrollZigzag(RIGHT, UP,
                 false); // right->right->upper left->right->...->prev page
  }
}

void Viewer::scrollForwardVerticalFirst() {
  if (!doubleMangaPage) {
    scrollZigzag(DOWN, RIGHT,
                 true); // down->down->upper right->down->...->next page
  } else {
    scrollZigzag(DOWN, LEFT,
                 true); // down->down->upper left->down->...->next page
  }
}
...
bool Viewer::isEdge(scrollDirection d) {
  if (d == UP)
    ...
    return horizontalScrollBar()->sliderPosition() ==
...
void Viewer::scrollZigzag(scrollDirection d1, scrollDirection d2,
                          bool forward) {
  if (!isEdge(d1)) {
    if (d1 == UP)
      scrollTo(horizontalScrollBar()->sliderPosition(),
               verticalScrollBar()->sliderPosition() -
                   static_cast<int>((height() * 0.80)));
    else if (d1 == DOWN)
      scrollTo(horizontalScrollBar()->sliderPosition(),
               verticalScrollBar()->sliderPosition() +
                   static_cast<int>((height() * 0.80)));
    ...
    if (d2 == UP)
      y = std::max(verticalScrollBar()->sliderPosition() -
                       static_cast<int>((height() * 0.80)),
                   verticalScrollBar()->minimum());
    else if (d2 == DOWN)
      y = std::min(verticalScrollBar()->sliderPosition() +
                       static_cast<int>((height() * 0.80)),
                   verticalScrollBar()->maximum());
    else if (d2 == LEFT)
      x = std::max(horizontalScrollBar()->sliderPosition() -
                       static_cast<int>((width() * 0.80)),
                   horizontalScrollBar()->minimum());
    else // d2 == RIGHT
      x = std::min(horizontalScrollBar()->sliderPosition() +
                       static_cast<int>((width() * 0.80)),
                   horizontalScrollBar()->maximum());
  ...
  if (render->hasLoadedComic()) {
    ...
    if (key == ShortcutsManager::getShortcutsManager().getShortcut(
      ...
      scrollBackwardVerticalFirst();
                        ...
                        MOVE_DOWN_ACTION_Y) ||
             key == ShortcutsManager::getShortcutsManager().getShortcut(
                        MOVE_UP_ACTION_Y) ||
             key == ShortcutsManager::getShortcutsManager().getShortcut(
                        MOVE_LEFT_ACTION_Y) ||
             key == ShortcutsManager::getShortcutsManager().getShortcut(
...
static void animateScroll(QPropertyAnimation &scroller,
                          const QScrollBar &scrollBar, int delta) {
  int deltaNotFinished = 0;
  if (scroller.state() == QAbstractAnimation::Running) {
    deltaNotFinished =
        scroller.startValue().toInt() - scroller.endValue().toInt();
    scroller.stop();
  }
...
void Viewer::wheelEvent(QWheelEvent *event) {
  if (render->hasLoadedComic()) {
    if (event->angleDelta().x() > 0) {
      animateScroll(*horizontalScroller, *horizontalScrollBar(),
                    event->angleDelta().x());
      return;
    }
    ...
    animateScroll(*verticalScroller, *verticalScrollBar(),
                  event->angleDelta().y());
  }
}
...
void Viewer::resizeEvent(QResizeEvent *event) {
  updateContentSize();
  goToFlow->updateSize();
  goToFlow->move((width() - goToFlow->width()) / 2,
                 height() - goToFlow->height());
...
void Viewer::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (magnifyingGlassShowed)
    mglass->move(static_cast<int>(event->x() - float(mglass->width()) / 2),
                 static_cast<int>(event->y() - float(mglass->height()) / 2));
  ...
  if (render->hasLoadedComic()) {
    if (showGoToFlowAnimation->state() != QPropertyAnimation::Running) {
      if (Configuration::getConfiguration().getDisableShowOnMouseOver() ==
        ...
        if (goToFlow->isVisible()) {
          ...
          if (gtfPos.y() < 0 || gtfPos.x() < 0 ||
              gtfPos.x() > goToFlow->width()) // TODO this extra check is for
                                              // Mavericks (mouseMove over
                                              // goToFlowGL seems to be broken)
            animateHideGoToFlow();
          // goToFlow->hide();
        } else {
          int umbral = (width() - goToFlow->width()) / 2;
          if ((event->y() > height() - 15) && (event->x() > umbral) &&
              (event->x() < width() - umbral)) {
    ...
    if (drag) {
      ...
      verticalScrollBar()->setSliderPosition(
          currentPosY = currentPosY + (yDragOrigin - event->y()));
      horizontalScrollBar()->setSliderPosition(
          currentPosX = currentPosX + (xDragOrigin - event->x()));
      yDragOrigin = event->y();
      xDragOrigin = event->x();
    }
  }
...
QPixmap Viewer::pixmap() { return content->pixmap(Qt::ReturnByValue); }
...
void Viewer::showMagnifyingGlass() {
  if (render->hasLoadedComic()) {
    ...
    mglass->show();
    mglass->updateImage(mglass->x() + mglass->width() / 2,
                        mglass->y() + mglass->height() / 2);
...
void Viewer::animateShowGoToFlow() {
  if (goToFlow->isHidden() &&
    ...
    disconnect(showGoToFlowAnimation, SIGNAL(finished()), goToFlow,
               SLOT(hide()));
    connect(showGoToFlowAnimation, SIGNAL(finished()), this,
            SLOT(moveCursorToGoToFlow()));
    showGoToFlowAnimation->setStartValue(
        QPoint((width() - goToFlow->width()) / 2, height() - 10));
    showGoToFlowAnimation->setEndValue(QPoint((width() - goToFlow->width()) / 2,
                                              height() - goToFlow->height()));
...
void Viewer::animateHideGoToFlow() {
  if (goToFlow->isVisible() &&
    ...
    disconnect(showGoToFlowAnimation, SIGNAL(finished()), this,
               SLOT(moveCursorToGoToFlow()));
    showGoToFlowAnimation->setStartValue(QPoint(
        (width() - goToFlow->width()) / 2, height() - goToFlow->height()));
    showGoToFlowAnimation->setEndValue(
        QPoint((width() - goToFlow->width()) / 2, height()));
    showGoToFlowAnimation->start();
    goToFlow->centerSlide(render->getIndex());
    goToFlow->setPageNumber(render->getIndex());
...
void Viewer::moveCursorToGoToFlow() {
  if (Configuration::getConfiguration().getDisableShowOnMouseOver()) {
    return;
  }

  // Move cursor to goToFlow widget on show (this avoid hide when mouse is
  // moved)
...
void Viewer::rotateLeft() { render->rotateLeft(); }
void Viewer::rotateRight() { render->rotateRight(); }
...
void Viewer::animateShowTranslator() {
  if (translator->isHidden() &&
      translatorAnimation->state() != QPropertyAnimation::Running) {
    disconnect(translatorAnimation, SIGNAL(finished()), translator,
               SLOT(hide()));
    ...
    if (translator->pos().x() < 0) {
      translatorAnimation->setStartValue(
          QPoint(-translator->width(), translator->pos().y()));
    } else {
      translatorAnimation->setStartValue(
          QPoint(width() + translator->width(), translator->pos().y()));
    }
...
bool Viewer::getIsMangaMode() { return doubleMangaPage; }

bool Viewer::event(QEvent *event) {
  if (event->type() == QEvent::Gesture) {

    QGestureEvent *GEvent = static_cast<QGestureEvent *>(event);
    if (QGesture *Gest = GEvent->gesture(Qt::PinchGesture)) {
      event->accept();
      return handlePinchGesture(static_cast<QPinchGesture *>(Gest));
    }
...
bool Viewer::handlePinchGesture(QPinchGesture *Gesture) {
  if (Gesture->state() == Qt::GestureFinished) {
    // auto reset zoom factor
    setZoomFactor(100);
    return true;
  }
  Gesture->setGestureCancelPolicy(QGesture::CancelAllInContext);
  // qDebug() << content->
  setZoomFactor(int(Gesture->totalScaleFactor() * 100));
  return true;
}
...
bool Viewer::handleTapGesture(QTapGesture *Gesture) {
  ...
  Gesture->setGestureCancelPolicy(QGesture::CancelAllInContext);
  QPoint TapPosInWidget = Gesture->position().toPoint();
  int W = width();
  int H = height();
  // LEFT
  if (QRect(0, 0, W / 2, H).contains(TapPosInWidget)) {
    left();
    return true;
  }
...
unsigned int Viewer::getIndex() { return render->getIndex() + 1; }

int Viewer::getCurrentPageNumber() { return render->getIndex(); }

void Viewer::updateComic(ComicDB &comic) {
  if (render->hasLoadedComic()) {
    // set currentPage
    if (!doublePage ||
        (doublePage && render->currentPageIsDoublePage() == false)) {
      comic.info.currentPage = render->getIndex() + 1;
    } else {
      if (!(render->getIndex() + 1 == comic.info.currentPage ||
            render->getIndex() + 2 == comic.info.currentPage)) {
        comic.info.currentPage =
            std::min(render->numPages(), render->getIndex() + 1);
      }
    }
    // set bookmarks
    ...
    QSettings settings(YACReader::getSettingsPath() + "/YACReader.ini",
                       QSettings::IniFormat);
